...
import a from "@/public/img/dashboard/wedding-hall.jpg";
import Link from "next/link";
import ToastSuccess from "@/components/toast/toast-success";
import { toast } from "sonner";

const TalarLayout = ({ children }) => {
    ...
    await axios
      ...
      .then((response) => {
        if (response.status === 200) {
          ...
          if (response?.data?.data?.role === "talar") {
            setIsAdmin(true);
          }
        }
      })
  ...
  return isLoading ? (
    ...
    >
      <div className="flex h-screen w-full flex-col items-center justify-center">
        <p className="rounded-lg bg-opacity-75 px-9 py-4 text-xl text-[#eee] backdrop-blur-lg">
