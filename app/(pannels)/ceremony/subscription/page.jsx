...
function SubScription() {
  ...
  const fetchCities = async () => {
    setIsLoading(true);
    await axios
      .get(`/api/ceremony/subscriptions?page=${searchParams.get("page") || 1}}`)
      .then((response) => {
  ...
  const fetchMySub = async () => {
    setIsLoading(true);
    await axios
      .get(`/api/ceremony/my-subscriptions`)
      .then((response) => {
    ...
    <div>
      <SubModal
        ...
        onConfirm={(id) => {
          ...
          window.location.href = `${backend}/business/payment/subscription/${id}?url=${front}${routes.ceremony.subscription.root}`;
        }}
        title="خرید اشتراک"
      ...
      <div className="rounded-xl bg-[#E7D7CA] bg-opacity-35 shadow-xl">
        <div className="mx-auto flex w-11/12 items-center justify-between gap-4 py-4">
          <div className="flex flex-col gap-2">
            <p className="text-xl font-bold">اشتراک تشریفات</p>
            <p>
              ...
              <span>
                {farsiNumber(
                  data2?.reserve?.remaining_days
                    ? data2?.reserve?.remaining_days
                    : 0 + data2?.current?.remaining_days
      ...
      <div className="mt-12 grid items-center justify-center gap-4 !rounded-xl max-md:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {data?.map((d) => (
          <div
            key={d.id}
            className="flex h-96 flex-col justify-between overflow-y-auto rounded-lg bg-[#F2F2F2] px-4 py-9"
          >
            ...
            <div className="my-9 flex items-center justify-between gap-4 px-4">
              <p>{farsiNumber(d.month_count)} ماه</p>
              <p className="">{persianPriceFormat(+d.price)} تومان</p>
            </div>
