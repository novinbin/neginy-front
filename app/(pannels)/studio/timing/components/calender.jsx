...
function Calender() {
  ...
  const fetchDates = async (start, end) => {
    setIsLoading(true);
    await axios
      .get(`/api/studio/occupied-dates?start=${start}&end=${end}`)
      .then((response) => {
  ...
  const onEditConfirm = async () => {
    try {
      ...
      const response = await axios.post(`/api/studio/occupied-dates`, {
  ...
  const onDelete = async () => {
    try {
      ...
      const response = await axios.delete(
        `/api/studio/occupied-dates?date=${gregorianDateTitle}`,
      );
