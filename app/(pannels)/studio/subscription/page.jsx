...
import { BadgeDollarSign, CalendarDays, Eye } from "lucide-react";
...
function SubScription() {
  ...
  const fetchCities = async () => {
    setIsLoading(true);
    await axios
      .get(`/api/studio/subscriptions?page=${searchParams.get("page") || 1}}`)
      .then((response) => {
  ...
  const fetchMySub = async () => {
    setIsLoading(true);
    await axios
      .get(`/api/studio/my-subscriptions`)
      .then((response) => {
    ...
    <div>
      <SubModal
        ...
        onConfirm={(id) => {
          ...
          window.location.href = `${backend}/business/payment/subscription/${id}?url=${front}${routes.studio.subscription.root}`;
        }}
        title="خرید اشتراک"
          ...
          >
            ...
            <div className="my-9 flex items-center justify-between gap-4 px-4">
              <p className="flex items-center gap-2">
                <CalendarDays className="text-[#d4b694]" />
                {farsiNumber(d.month_count)} ماه
              </p>
              <p className="flex items-center gap-2">
