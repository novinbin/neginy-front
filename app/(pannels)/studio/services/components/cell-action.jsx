...
const CellAction = ({ data }) => {
  ...
  const onDelete = async () => {
    try {
      ...
      const response = await axios.delete(`/api/studio/my-services/${data.id}`);
  ...
  return (
    <div className="flex items-center justify-center gap-1">
      ...
      <DetailsModal
        ...
        data={data}
      />
      <Link href={routes.studio.services.details(data.id)}>
      ...
      <Link href={routes.studio.services.edit(data.id)}>
        <div className="cursor-pointer rounded-md p-1 transition-all duration-200 hover:bg-muted">
