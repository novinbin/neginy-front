...
const StatusAction = ({ data }) => {
  ...
  const [status, setStatus] = useState(
    data?.business_info?.approved ? "تایید شده" : "تایید نشده",
  );
  ...
  const changeAccess = async (accessName, accessValue) => {
    ...
    try {
      const response = await axios.post(
        `/api/admin/users/${data.id}/change-status`,
        {
      ...
      if (response.status === 200) {
        toast.success(<ToastSuccess text={"کاربر با موفقیت تایید شد"} />);
        setStatus(accessName);
      }
    } catch (error) {
      toast.error(<ToastError text={error?.response?.data?.message} />);
    } finally {
      setLoading(false);
    }
  };
