...
function Calender() {
  ...
  const fetchDates = async (start, end) => {
    setIsLoading(true);
    await axios
      .get(`/api/talar/occupied-dates?start=${start}&end=${end}`)
      .then((response) => {
  ...
  const onEditConfirm = async () => {
    try {
      ...
      const response = await axios.post(`/api/talar/occupied-dates`, {
  ...
  const onDelete = async () => {
    try {
      ...
      const response = await axios.delete(
        `/api/talar/occupied-dates?date=${gregorianDateTitle}`,
      );
